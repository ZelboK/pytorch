...
__device__ inline thrust::pair<int64_t, int64_t> get_index_mapping1d(
    int64_t input_w,
    int64_t output_w,
    int64_t output_x,
    int64_t pad_l) {
  // 3D grid of 1D blocks
  auto input_offset = (blockIdx.y + blockIdx.z * gridDim.y) * input_w;
  auto output_offset = (blockIdx.y + blockIdx.z * gridDim.y) * output_w;

  auto i_start_x = ::max(int64_t(0), -pad_l);
  auto o_start_x = ::max(int64_t(0), pad_l);

  int64_t input_x = ::abs(output_x - pad_l) -
      ::abs(output_x - (input_w + pad_l - 1)) - output_x + 2 * pad_l + input_w -
      1 - o_start_x + i_start_x;
...
__device__ inline thrust::pair<int64_t, int64_t> get_index_mapping2d(
  ...
  auto input_offset =
      ((blockIdx.y + y_shift) + (blockIdx.z + z_shift) * nplane) * input_dim_x *
      input_dim_y;
  auto output_offset =
      ((blockIdx.y + y_shift) + (blockIdx.z + z_shift) * nplane) *
...
// if(output_xy == 26 && print) {
//     auto value = thrust::make_pair<int64_t, int64_t>(
//       input_offset + input_y * input_dim_x + input_x,
//       output_offset + output_y * output_dim_x + output_x);
    
//     printf("inp: %ld out: %ld \n", value.first, value.second);
//   }
auto value = thrust::make_pair<int64_t, int64_t>(
      input_offset + input_y * input_dim_x + input_x,
      output_offset + output_y * output_dim_x + output_x);
      if(value.second == 26 || value.second == 28) {
        printf("inp: %ld out: %ld \n", value.first, value.second);
        printf("ri: %ld ci: %ld ro: %ld co% ld\n", input_y, input_x, output_y, output_x);
      }
  return thrust::make_pair<int64_t, int64_t>(
      input_offset + input_y * input_dim_x + input_x,
      output_offset + output_y * output_dim_x + output_x);
...
// constrain the border padding on CPU
template <typename scalar_t>
...
__global__ void reflection_pad2d_backward_out_kernel(
  ...
  if (output_xy < output_dim_x * output_dim_y) {
    ...
    // if(output_xy == 28) { 
    //   printf("FROM 28: %u\n", output_xy);
    //   printf("GRAD OUT FROM 28: %f \n", grad_output[index_pair.second]);
    //   printf("28: input_i: %ld output_i %ld\n", index_pair.first, index_pair.second);
    //   int input_row = index_pair.first / input_dim_x;
    //   int input_col = index_pair.first % input_dim_x;

    //   printf("28: Input row: %d, Input col: %d\n", input_row, input_col);
    // }
    if (index_pair.first == debug_thread) {
      printf("output_xy: %u\n", output_xy);
      printf("grad_output: %f \n", grad_output[index_pair.second]);
      printf("input_i: %ld output_i %ld\n", index_pair.first, index_pair.second);
      int input_row = index_pair.first / input_dim_x;
      int input_col = index_pair.first % input_dim_x;

      printf("Input row: %d, Input col: %d\n", input_row, input_col);
    }
    gpuAtomicAddNoReturn(
        &grad_input[index_pair.first], grad_output[index_pair.second]);
  }
...
__device__ inline void parallel_reflection_pad3d(
  ...
  int64_t input_x = ::abs(output_x - pad_left) -
      ::abs(output_x - (input.size(4) + pad_left - 1)) - output_x +
      2 * pad_left + input.size(4) - 1 - o_start_x + i_start_x;
  int64_t input_y = ::abs(output_y - pad_top) -
      ::abs(output_y - (input.size(3) + pad_top - 1)) - output_y + 2 * pad_top +
      input.size(3) - 1 - o_start_y + i_start_y;

  int64_t input_z = ::abs(output_z - pad_front) -
      ::abs(output_z - (input.size(2) + pad_front - 1)) - output_z +
...
(const Tensor& input_, IntArrayRef padding, const Tensor& output) {
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND2(
      kHalf, kBFloat16, input.scalar_type(), "reflection_pad3d_out_cuda", [&] {
        ...
        int64_t output_plane_size = output_packed.size(2) *
            output_packed.size(3) * output_packed.size(4);
